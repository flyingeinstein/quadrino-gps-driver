...
typedef struct {
    ...
    uint16_t week; // gps week number since 1/1/1970
    uint32_t time; // gps time of week as 1/100th of a second since beginning of week (Sunday start)
} GPS_DETAIL;



typedef struct {
  uint16_t    date;         // days since Jan 1, 1980
  uint32_t    time;         // 1.100th seconds since midnight
} GPS_DATETIME;
...
#define I2C_GPS_WEEK                                37   // UTC GPS Week number (Weeks since Jan 1, 1980)
#define I2C_GPS_TIME                                39   // UTC GPS Time as 1/100th of a second since the beginning of the week
...
#endif // __QUADRINO_GPS_REGISTERS_H
